...
const dueTasks: MaintenanceTask[] = [
    // 2 VENCIDOS
    { id: 'MT-VNC-001', type: 'Preventiu', title: 'Neteja de pupitre i validadora', vehicleId: findVehicleByOperatorAndIndex('op-01', 4).id, equipmentType: 'Pupitre', frequency: 'Mensual', dueDate: sub(today, { days: 3 }), status: 'Vençut', operatorId: 'op-01', lastMaintenanceDate: sub(today, { days: 33 }), km: 125000 },
    { id: 'MT-VNC-002', type: 'Preventiu', title: 'Neteja d\'autocutter i connectors', vehicleId: findVehicleByOperatorAndIndex('op-11', 3).id, equipmentType: 'Placa conexiones', frequency: 'Trimestral', dueDate: sub(today, { days: 8 }), status: 'Vençut', operatorId: 'op-11', lastMaintenanceDate: sub(today, { days: 98 }), km: 85000 },
    
    // 8 Mensuales (vencen en 1-15 días)
    ...Array.from({ length: 8 }, (_, i) => {
        const opId = getRandomItem(validOperatorIds);
        return {
            id: `MT-MNS-${i+1}`, type: 'Preventiu' as const, title: 'Neteja de pupitre i validadora' as const, vehicleId: findVehicleByOperatorAndIndex(opId, i).id, equipmentType: 'Limpieza Validadora' as EquipmentType, frequency: 'Mensual' as const, 
    ...
    ...Array.from({ length: 6 }, (_, i) => {
        const opId = getRandomItem(validOperatorIds);
        return {
            id: `MT-TRS-${i+1}`, type: 'Preventiu' as const, title: 'Neteja d\'autocutter i connectors' as const, vehicleId: findVehicleByOperatorAndIndex(opId, 8 + i).id, equipmentType: 'Soporte y Barras' as EquipmentType, frequency: 'Trimestral' as const,
    ...
    ...Array.from({ length: 4 }, (_, i) => {
        const opId = getRandomItem(validOperatorIds);
        return {
            id: `MT-SMS-${i+1}`, type: 'Preventiu' as const, title: 'Verificació general del sistema' as const, vehicleId: findVehicleByOperatorAndIndex(opId, 14 + i).id, equipmentType: 'Antena' as EquipmentType, frequency: 'Semestral' as const,
...
const completedTasks: MaintenanceTask[] = [
    { id: 'MT-C01', type: 'Preventiu', title: 'Neteja de pupitre i validadora', vehicleId: findVehicleByOperatorAndIndex('op-01', 0).id, equipmentType: 'Pupitre', frequency: 'Mensual', dueDate: sub(today, { days: 32 }), status: 'Completat', technician: 'Jordi' },
    { id: 'MT-C02', type: 'Preventiu', title: 'Neteja d\'autocutter i connectors', vehicleId: findVehicleByOperatorAndIndex('op-02', 1).id, equipmentType: 'Limpieza Validadora', frequency: 'Trimestral', dueDate: sub(today, { days: 80 }), status: 'Completat', technician: 'Miquel' },
    { id: 'MT-C03', type: 'Preventiu', title: 'Verificació general del sistema', vehicleId: findVehicleByOperatorAndIndex('op-03', 1).id, equipmentType: 'Todos', frequency: 'Anual', dueDate: sub(today, { months: 11 }), status: 'Completat', technician: 'Pau' },
    { id: 'MT-C04', type: 'Preventiu', title: 'Revisió de fixació d\'antenes i barres', vehicleId: findVehicleByOperatorAndIndex('op-11', 1).id, equipmentType: 'Antena', frequency: 'Semestral', dueDate: sub(today, { months: 5 }), status: 'Completat', technician: 'Carles' },
    { id: 'MT-C05', type: 'Preventiu', title: 'Substitució de piles CR-2032', vehicleId: findVehicleByOperatorAndIndex('op-05', 0).id, equipmentType: 'Baterías CPU', frequency: 'Bianual', dueDate: sub(today, { months: 2 }), status: 'Completat', technician: 'Oriol' },
    { id: 'MT-C06', type: 'Preventiu', title: 'Verificació de voltatges i fusibles', vehicleId: findVehicleByOperatorAndIndex('op-03', 2).id, equipmentType: 'Soporte y Barras', frequency: 'Anual', dueDate: sub(today, { weeks: 2 }), status: 'Completat', technician: 'David' },
    { id: 'MT-C07', type: 'Preventiu', title: 'Substitució de piles CR-2032', vehicleId: findVehicleByOperatorAndIndex('op-02', 5).id, equipmentType: 'Brida y Antena', frequency: 'Bianual', dueDate: sub(today, { months: 1, days: 1 }), status: 'Completat', technician: 'Bernat' },
    { id: 'MT-C08', type: 'Preventiu', title: 'Neteja d\'autocutter i connectors', vehicleId: findVehicleByOperatorAndIndex('op-01', 5).id, equipmentType: 'Placa conexiones', frequency: 'Trimestral', dueDate: sub(today, { days: 10 }), status: 'Completat', technician: 'Quim' },
    { id: 'MT-C09', type: 'Preventiu', title: 'Verificació de voltatges i fusibles', vehicleId: findVehicleByOperatorAndIndex('op-06', 2).id, equipmentType: 'Soporte y Barras', frequency: 'Anual', dueDate: sub(today, { weeks: 1 }), status: 'Completat', technician: 'Toni' },
    { id: 'MT-C10', type: 'Preventiu', title: 'Revisió de fixació d\'antenes i barres', vehicleId: findVehicleByOperatorAndIndex('op-09', 2).id, equipmentType: 'Antena', frequency: 'Bianual', dueDate: sub(today, { weeks: 6 }), status: 'Completat', technician: 'Ramon' },
];
...
export const mockIncidents: Incident[] = [
    ...
    { id: 'AV-002', vehicleId: findVehicleByOperatorAndIndex('op-02', 3).id, issue: 'El pupitre no s\'encén o no obre serveis', priority: 'Crítica', status: 'Abierto', assignedTo: null, operatorId: 'op-02', reportedAt: sub(today, { hours: 4 }), slaDays: 1, equipmentType: 'Pupitre', location: 'Taller 2' },
    { id: 'AV-003', vehicleId: findVehicleByOperatorAndIndex('op-11', 4).id, issue: 'El sistema de validació no comunica', priority: 'Alta', status: 'Abierto', assignedTo: null, operatorId: 'op-11', reportedAt: sub(today, { hours: 8 }), slaDays: 1, equipmentType: 'Validadora INDRA', location: 'Taller 1' },
    { id: 'AV-004', vehicleId: findVehicleByOperatorAndIndex('op-03', 3).id, issue: 'Validadora no llegeix targetes T-mobilitat', priority: 'Alta', status: 'Abierto', assignedTo: null, operatorId: 'op-03', reportedAt: sub(today, { hours: 10 }), slaDays: 2, equipmentType: 'Validadora Inetum', location: 'Taller 3' },
    
    // 3 Abiertas - NORMAL
    { id: 'AV-005', vehicleId: findVehicleByOperatorAndIndex('op-01', 7).id, issue: 'La impressora del pupitre no imprimeix tiquets', priority: 'Normal', status: 'Abierto', assignedTo: null, operatorId: 'op-01', reportedAt: sub(today, { days: 1 }), slaDays: 3, equipmentType: 'Pupitre', location: 'En Ruta' },
    { id: 'AV-006', vehicleId: findVehicleByOperatorAndIndex('op-05', 2).id, issue: 'El sistema de validació no descompta viatges', priority: 'Normal', status: 'Abierto', assignedTo: null, operatorId: 'op-05', reportedAt: sub(today, { days: 1, hours: 2 }), slaDays: 3, equipmentType: 'Validadora Inetum', location: 'En Ruta' },
    { id: 'AV-007', vehicleId: findVehicleByOperatorAndIndex('op-02', 4).id, issue: 'El pupitre no té cobertura 4G/GPS', priority: 'Normal', status: 'Abierto', assignedTo: null, operatorId: 'op-02', reportedAt: sub(today, { days: 1, hours: 5 }), slaDays: 2, equipmentType: 'Antena', location: 'Taller 2' },
    
    // 3 En Progreso
    { id: 'AV-008', vehicleId: findVehicleByOperatorAndIndex('op-11', 5).id, issue: 'La impressora del pupitre no talla el paper', priority: 'Normal', status: 'En Progreso', assignedTo: 'Jordi', operatorId: 'op-11', reportedAt: sub(today, { days: 2 }), slaDays: 4, equipmentType: 'Pupitre', location: 'Taller 1' },
    { id: 'AV-009', vehicleId: findVehicleByOperatorAndIndex('op-01', 8).id, issue: 'Validadora vandalitzada', priority: 'Baixa', status: 'En Progreso', assignedTo: 'Carles', operatorId: 'op-01', reportedAt: sub(today, { days: 2 }), slaDays: 5, equipmentType: 'Validadora INDRA', location: 'Taller 3' },
    { id: 'AV-010', vehicleId: findVehicleByOperatorAndIndex('op-03', 4).id, issue: 'Curtcircuit en el cablejat de la validadora', priority: 'Alta', status: 'En Progreso', assignedTo: 'Pau', operatorId: 'op-03', reportedAt: sub(today, { days: 1 }), slaDays: 2, equipmentType: 'Validadora INDRA', location: 'Taller 2' },
    
    // 2 Resueltos
    { id: 'AV-011', vehicleId: findVehicleByOperatorAndIndex('op-02', 6).id, issue: 'Fallo en els fusibles del sistema embarcat', priority: 'Alta', status: 'Resuelto', assignedTo: 'Miquel', operatorId: 'op-02', reportedAt: sub(today, { days: 3 }), slaDays: 2, equipmentType: 'Cableado', location: 'Taller 2' },
    { id: 'AV-012', vehicleId: findVehicleByOperatorAndIndex('op-09', 0).id, issue: 'Validadora no s\'encén', priority: 'Normal', status: 'Resuelto', assignedTo: 'David', operatorId: 'op-09', reportedAt: sub(today, { days: 4 }), slaDays: 5, equipmentType: 'Validadora INDRA', location: 'Cotxera' },
];
